...
class permissionManageStore {
    ...
    @action toLookUserByRole = (event, record) => {
        this.customerHashPush('permissionManage/lookUserByRole',record)
    }
    ...
    @action saveButtonPermissions = async (nextTargetKeys, direction, moveKeys) => {

        let params = {
            data: {
                role_code: this.currentRole.role_code,
                button_id_list: moveKeys,
                state: direction == 'right' ? 'insert' : 'delete'
            },
            method: 'POST'
        };
        let res = await api.permission.saveButtonPermissions(params);
        if (res.code == 200) {
            this.selectTartgetButtonkeys = nextTargetKeys;
            message.success('菜单分配成功！');
            await this.getRoleButtonList();
            await this.getAllButtonList();
            return;
        }
        message.success('菜单分配失败！');
    };
    ...
    @action getAllButtonList = async () => {

        let res = await api.permission.getAllButtonList();
        if (res.code == 200) {
            this.selectTartgetButtonkeys.map(key => {
                res.data.filter(item => item.key == key)
            })
    ...
    @action saveMenuHandle = async event => {
        ...
        if (this.modalTitle == '编辑菜单') {
            if(params.data.text==""||params.data.menu==""||params.data.router==""||params.data.type==""){
    ...
    @action saveButtonHandle = async event => {
        ...
        if (this.modalTitle == '编辑按钮') {
            if(params.data.name==""||params.data.button_code==""||params.data.icon==""||params.data.file_path==""||params.data.entry_function==""||params.data.is_standard_button==""){
                message.error('请将必填信息填写完整')
            }else{
