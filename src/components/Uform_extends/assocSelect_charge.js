...
export default class AssocSelect_charge extends React.Component {
    constructor(props) {
        super(props)
    ...
    async simulateClick() {
        ...
        // 1、数据为空，
        if (!this.store.chargeRowData) {
            return;
        }


        // 2、编辑状态
        if (this.store.chargeOption == 'edit') {
            for (let i = 0; i < this.store.triggers.length; i++) {
                ...
                if (element.props.query_cfg.trigger_group_uuid == this.props.query_cfg.trigger_group_uuid) {
                    ...
                    let curren_value = element.store.chargeRowData['ghost_' + element.props['x-props'].ass_select_field_id];
                    await element.getDefaultOptionList(element);
                    element.props.onChange(curren_value, element.props['x-props'].ass_select_field_id)
        ...
        for (let i = 0; i < this.store.triggers.length; i++) {
            ...
            // 2、同一组
            if (element.props.query_cfg.trigger_group_uuid == this.props.query_cfg.trigger_group_uuid) {
                if (element.props.chargeOption == 'edit') {
                    element.initValues()
                }
            }
        }
    ...
    getPrevSelValue(current_ele) {
        ...
        for (let i = 0; i < this.store.triggers.length; i++) {
            ...
            let prev_value = element.store.chargeRowData['ghost_' + element.props['x-props'].ass_select_field_id];
    ...
    async onSelect(value) {
        // 1、设置当前字段的value
        this.props.onChange(value, this.props['x-props'].ass_select_field_id)
        ...
        for (let i = 0; i < this.store.triggers.length; i++) {
            ...
            if (element.props.query_cfg.trigger_group_uuid == this.props.query_cfg.trigger_group_uuid) {
                ...
                if (element.props.query_cfg.level - this.props.query_cfg.level == 1) {
                    ...
                    element.props.value && element.props.onChange('', element.props['x-props'].ass_select_field_id);
                }
    ...
    getOptionValue() {
        ...
        // console.log('查看props', this.props.value);
        if (!this.props.value) {
            return;
        }
