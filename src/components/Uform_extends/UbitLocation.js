...
import { Modal, Table, message, Form, Button, Input, Icon, Select } from 'antd';
...
export default class UbitLocation extends React.Component {
    ...
    getcabinetinfo(contract_no) {
        ...
        for (var i = 0; i < customeruibt.length; i++) {
            for (var j = 0; j < cabinetdasoure.length; j++) {
                if (customeruibt[i].cabinet_ulocs.indexOf(cabinetdasoure[j].cabinet_uloc) != -1) {
                    cabinetdasoure[j].disabled = true;
                    if (customeruibt[i].cabinet_ulocs.indexOf(cabinetdasoure[j].cabinet_uloc) == 0) {
        ...
        return this.state.UbitDataview.map((item, index) => {
            return (
                <div key={index}>
                    ...
                    <span style={{ cursor: 'pointer' }} onClick={() => this.deleterow(item)} style={{ marginLeft: '30px' }}>
    ...
    getcabinet() {
        ...
        return (
            <Select value={this.state.cabinetValue} onChange={this.handleChange}>
                {cabinetarr.map((item) => {
                    return (
    ...
    render() {
        ...
        return (
            <div>
                ...
                {this.getrowData()}
                <Modal title="分配机位" onOk={this.handleOk} okText="确定" onCancel={this.handleCancel} width="800px" visible={this.state.visible}>
                    ...
                    <Form {...layout} style={{ marginTop: '15px' }} ref="formm">
                        <Form.Item label="设备名称" name="email">
                            <Input value={this.state.formvalue} onChange={(event) => this.formChange(event)} placeholder="请输入正确设备名称" />
                        </Form.Item>
                    </Form>
                </Modal>
            </div>
