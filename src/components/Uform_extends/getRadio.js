    ...
    render() {
        ...
        return (
            <div style={{ marginBottom: '10px' }}>
                {this.state.checkboxList.length > 0 ? <Radio.Group value={selectedvalue} onChange={this.onChange}>
                    {
                        this.state.checkboxList.map((item,index) => {
                            return <Radio style={{ display:'block',margin:'0px' }} key={index} value={item}>{item}</Radio>
                        })
                    }
                </Radio.Group> : null}
