...
export default class Dropdowncombox extends React.Component {
	...
	componentDidMount() {
		...
		if (
			this.props.query_cfg.level ==
			group[this.props.query_cfg.trigger_group_uuid]
		) {
			//模拟用户选择
			this.simulateClick();
		}
	...
	async simulateClick() {
		...
		if (this.store.table_action == "edit_table") {
			for (let i = 0; i < this.store.triggers.length; i++) {
				...
				// 2、同一组
				if (
					element.props.query_cfg.trigger_group_uuid ==
					this.props.query_cfg.trigger_group_uuid
				) {
					let curren_value =
						element.store.selectedRows[0][
							"ghost_" + element.props.ass_select_field_id
						];
	...
	async getDefaultOptionList(current_ele, element) {
		...
		await element.getOptionList(
			element.props.query_cfg,
			prev_sel_value,
			element,
		);
	}
	...
	getGroupCount() {
		...
		Object.keys(this.store.formCfg.properties.group_all.properties).map(
			(gourp_key) => {
				let fields_group =
					this.store.formCfg.properties.group_all.properties[gourp_key];
				...
				for (let key in fields_group.properties) {
					...
					if (
						item["x-props"] &&
						item["x-props"].query_cfg &&
						item["x-props"].query_cfg.level
					) {
	...
	getPrevSelValue(current_ele) {
		for (let i = 0; i < this.store.triggers.length; i++) {
			...
			if (
				element.props.query_cfg.trigger_group_uuid !=
				current_ele.props.query_cfg.trigger_group_uuid
			) {
				continue;
			}
			// 不是上一个联动的结束本次循环
			if (
				current_ele.props.query_cfg.level - element.props.query_cfg.level !=
				1
			) {
				continue;
			}
	...
	getOptionList = async (query_cfg, value, element) => {
		...
		console.log(params);
		const res = await api.activity.getAssociateData(params);
		if (res.code === 200) {
			const optionList = this.formatOptionList(
				res.data,
