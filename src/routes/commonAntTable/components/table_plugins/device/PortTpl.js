...
export default class PortTpl extends React.Component {
    ...
    render() {
        ...
        return (
            <Modal visible={this.state.visible} onCancel={() => this.onCancel()} onOk={() => this.SavePortsHandler()} destroyOnClose={true} width={1400} title="批量添加端口">
                <div>{this.deviceInfo()}</div>
                ...
                <div style={wrapper}>
                    ...
                    */}
                        <div style={{ marginLeft: '10px' }}>
                            <Input placeholder="端口数量" onChange={(event) => this.onChangeNumberrequest(event.target.value)} />
                        </div>
                        <div style={{ marginLeft: '10px' }}>
                ...
                <div>
                    {alllines.length > 0 &&
                        alllines.map((item, key) => (
                            <PortLine
                                key={key}
                                statics={this.statics}
                                generate_Displayports={this.generate_Displayports}
                                destoryLine={this.DeleteLine}
                                nameTpl={this.state.nameTpl}
