...
export default class Pathwrapper extends React.Component {
    ...
    saveFormData = async () => {
        ...
        let res = this.props.form_option === "add" 
        ? 
        await api.device_api.addBossXpath(params) 
        : 
        await api.device_api.updateBossXpath(params)
        if (res.code == 200) {
            ...
            <div>
                <Row>
                    <Col span={22} style={{paddingTop: '10px'}}>
                        <SchemaForm
                            value = {
                                ...
                                ?
                                {xpathname: this.props.formData.xpathname}
                                :
                                {}
                            }
                            labelCol={7}
                            ...
                            wrapperCol={15}
                        >
                            <Field required title="跳纤名称" type="string" name="xpathname" 
                            x-props = {{
                                style: {
                                    width: '100%'
                                },
                    ...
                    <Col span={2} style={addCol} onClick={event => this.addPoint()} >
                        <Icon type="folder-add" style= {addIcon} /><br />
