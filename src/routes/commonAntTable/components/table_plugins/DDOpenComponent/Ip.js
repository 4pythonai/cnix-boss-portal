...
const Ip = ({ appendrows, catid, product_name, bizCode }) => {
	...
	const updateTotalStr = (addresses) => {
		// 使用 Set 去重
		const uniqueIps = [
			...new Set(addresses.map((item) => item.ipaddress).filter((ip) => ip)),
		];
	...
	const callAppendrows = () => {
		// 同样在这里也要确保使用去重后的IP列表
		const uniqueIps = [
			...new Set(ipAddresses.map((item) => item.ipaddress).filter((ip) => ip)),
		];
	...
	return (
		<div>
			...
			<Button onClick={addIP} style={{ marginBottom: "16px" }}>
				增加IP
			</Button>
			...
			{ipAddresses.map((item, index) => (
				...
				>
					<Button danger onClick={() => delIP(index)}>
						删除
					</Button>
					<AutoComplete
						style={{ width: "100%" }}
