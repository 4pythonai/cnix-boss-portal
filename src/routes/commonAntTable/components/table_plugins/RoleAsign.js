...
export default class RoleAsign extends React.Component {
    ...
    async SaveHandler() {
        ...
        let res = await api.user.saveRoleAssign(params);
        if (res.code == 200) {
    ...
    render() {
        ...
        return (
            <Modal visible={this.state.visible} onCancel={() => this.onCancel()} onOk={() => this.SaveHandler()} style={{ width: '400px' }} title="用户角色分配">
                <div>
                    <br />
                    <br />
