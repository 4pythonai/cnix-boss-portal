...
@observer
export default class GenerateNetworkCompleteOrder extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            visible: false,
     ...
     init() {
        ...
        if(this.props.commonTableStore.selectedRows[0].processDefinitionKey!='idc_network_op_order'){
            message.error('请选择网络开工单')
            return
        }
        this.fetchOrderInfo()
    ...
    async fetchOrderInfo() {
        ...
        let res = await api.acceptance.getNetowkrOrderData(params)
    ...
    saveFormData = async (values) => {
        ...
        let res = await api.acceptance.addNetworkCompleteOrder(params)
        if (res.code == 200) {
            this.setState({
                visible: false
            })
        ...
        if (this.state.rowkey.length > 1) {
            ...
            >
                ...
                <div style={{fontWeight: 'bold',margin:'10px 0px' }}>工单信息</div>
                    ...
                    <div style={{fontWeight: 'bold',margin:'10px 0px' }}>网络部操作记录</div>
                    {
