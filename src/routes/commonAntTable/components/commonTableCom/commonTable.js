...
export default class CommonTable extends React.Component {
    ...
    listData = async () => {
        console.log('listData>>>>>>>>>>>>>>', this.state.query_cfg);
        let params = {
            data: {
                actcode: this.commonTableStore.action_code,
                role: sessionStorage.getItem('role_code'),
                user: sessionStorage.getItem('user'),
                query_cfg: createQueryConfig(this.state.search_query_cfg, this.state.query_cfg),
    ...
    getFilterButton = () => {
        ...
        return this.state.isFilterSelfData ? (
            <Icon title="查看所有数据" style={filter_style} type="filter" theme="filled" onClick={() => this.filterSelfList(false)} />
        ) : (
