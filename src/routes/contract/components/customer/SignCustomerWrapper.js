...
export default class SignCustomerWrapper extends React.Component {
    ...
    getAddressList = async (customerId,customer_index) => {
        // @@ 远程获取客户地址列表
    ...
    getCurrentCustomer = async (customerId, customer_index, selectedCustomer) => {
        ...
        let addressMsg = customerId == '请选择'
            ...
            :
            await this.getAddressList(customerId,customer_index);
        let singers_customers = [...this.store.saveContractData.singers_customers]
        ...
        singers_customers[customer_index] = {
            ...
            addressId: addressMsg.addressList.length == 1 ? addressMsg.addressList[0].id : ''
        }

        this.store.setSingersCustomers(singers_customers)
    }

    getCurrentCustomerAddr = async (addressId, customer_index) => {
        let singers_customers = [...this.store.saveContractData.singers_customers]
    ...
    render() {
        return <div>
            {this.store.saveContractData.singers_customers.map((item, index) => (
                <SignCustomer
            ...
            <div className="" style={{ textAlign: 'center' }}>
                {
                    this.store.disabledSigners ? null :
                        <Icon
