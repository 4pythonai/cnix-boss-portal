...
class ShowUserRoleStore {
    ...
    @action clearUserOrRole = () => (this.userList = []);
...
export default class showMenuUser extends React.Component {
    ...
    getUserList = async (isFresh) => {
        ...
        this.store.clearUserOrRole();
        let res = this.store.activeKey == 1 ? await api.permission.getRoleByMenuId(params) : await api.permission.getUserInformationByMenuId(params);
        ...
        return this.store.userList.map((user) => {
            ...
            return (
                <Card
                    ...
                    cover={<img style={{ width: '100px', height: '100px', margin: '0 auto' }} alt="example" src={user.head_portrait} />}>
    ...
    getTabContent = () => {
        return (
            <div>
                ...
                <div style={{ textAlign: 'center' }}>{this.store.isEnd || <Icon type="more" style={{ fontSize: '24px' }} onClick={this.getMoreUsers} />}</div>
            </div>
        );
    };
