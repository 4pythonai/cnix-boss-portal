...
export default class showMenuUser extends React.Component {
    ...
    getUserList = async (isFresh) => {
        ...
        let res = this.store.activeKey == 1
            ?
            await api.permission.getRoleByMenuId(params)
            :
            await api.permission.getUserInformationByMenuId(params)
    ...
    getUserOrRole() {
        ...
        if (this.store.activeKey == 1) { // role
        ...
        return this.store.userList.map(user => { // user
            return <Card
                key={user.id}
                hoverable
    ...
    render() {
        ...
        return <Modal {...modalProps}>
            ...
            <Tabs activeKey={this.store.activeKey} onChange={this.onChangeTabPane}>
                <TabPane
                    tab={<span> <Icon type="user" /> 角色 </span>}
                    key="1">
                    {this.getTabContent()}
                </TabPane>
                <TabPane
                    tab={<span> <Icon type="android" /> 人员 </span>}
                    key="2" >
                    {this.getTabContent()}
                </TabPane>
            </Tabs>
