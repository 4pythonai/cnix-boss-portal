    ...
    getOptionButtons(record) {
        return (<div className="options">
            <Button
                ...
                size="small"
                type="primary" >分配菜单</Button>
            <Button
                className={ record.key == this.state.selectKey ? "marginRihgt10" : "marginRihgt10" }
                onClick={ event => this.store.allocationButton(event, record) }
                size="small"
                type="primary" >分配按钮</Button>
            <Button
                className={ record.key == this.state.selectKey ? "marginRihgt10" : "marginRihgt10" }
                onClick={ event => this.store.toPermissionDetail(event, record) }
                size="small"
                type="primary" >查看权限</Button>
            <Button
                className={ record.key == this.state.selectKey ? "marginRihgt10" : "marginRihgt10" }
                onClick={ event => this.store.toLookUserByRole(event, record) }
                size="small"
                type="primary" >查看人员</Button>
            <Button
                className={ record.key == this.state.selectKey ? "marginRihgt10" : "marginRihgt10" }
    ...
    getSearchRoleButtonGrp() {
        return <div className="roleButtonGroup">
            <Button
                ...
                size="small"
                type="primary"
                style={ { margin: '15px 20px' } }>新增角色</Button>
