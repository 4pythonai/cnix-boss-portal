...
export default class Tablesort extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      action_code:this.props.current_actcode
    };
    
  }
  componentDidUpdate(props){
      if(props.current_actcode!=this.state.action_code){
  ...
  async savedata(){
    ...
    let params={
        data:{
            action_code:this.state.action_code,
            filedids:dataarr
        }, method: 'POST'
    }
  ...
  render() {
    ...
    return (
      <DndProvider backend={HTML5Backend}>
        ...
        />
        <Button style={{display:'block',margin:'20px auto'}} onClick={()=>this.savedata()} type='primary'>保存</Button>
    </DndProvider>
    );
  }
}
