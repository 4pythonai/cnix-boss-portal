...
import { Form, Input, Divider, Card, Radio, Select, Row, Col, Button, message } from 'antd';
        ...
        this.props.form.validateFieldsAndScroll(async (err, values) => {
            if (!err) {
                if (this.dmstore.current_actcode === '') {
                    ...
                    let obj = {}
                    obj = Object.assign({ 'reftype': 'grid-service', maintable: this.dmstore.maintable, act_name: this.dmstore.current_actname, act_code: this.dmstore.current_actcode }, values);
                    let params = {
                        data: obj,
                        method: 'POST'
                    };
                    ...
                    if (this.props.tbref.current) {
                        let query_cfg = { count: 1, lines: { and_or_0: "and", field_0: "act_code", operator_0: "=", vset_0: this.dmstore.current_actcode } }
        ...
        return (
            <div>
                <Card title="Service说明" style={ { width: 1000 } }>
                    <div>在编辑或增加数据时候,有时候需要参考其他表格数据,可以配置一个actioncode,
