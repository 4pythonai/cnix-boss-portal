...
import { Select, Row, Col, Form, message, Input, Divider, Button } from 'antd';
...
export default class TriggerAdder extends React.Component {
    ...
    increateCounter(a, b, c) {
        ...
        this.state.lines.push(
            <Line triggerlineStore={ tstore } descCounter={ this.descCounter.bind(this) } idx={ this.state.linecounter + 1 } key={ this.state.linecounter + 1 } />
        );
    ...
    saveTriggerGroup() {
        ...
        triggerObj.actcode = this.dmstore.current_actcode
        triggerObj.lines = []
        this.state.lines.forEach(element => {
            let ts = element.props.triggerlineStore
            triggerObj.lines.push({ combo_table: ts.combo_table, field_e: ts.field_e, list_field: ts.list_field, value_field: ts.value_field, filter_field: ts.filter_field })
        });

        this.dmstore.saveTriggerGroup(triggerObj)
    }
    ...
    render() {
        return (
            <div style={ { marginLeft: '40px' } }>
                ...
                <div style={ { marginBottom: '10px' } }> <Input onChange={ event => this.setGroupName(event) } /></div>
                <Row>
                    <Col span={ 2 }>
