...
import { Select, Row, Col, Card, Form, message, Input, Divider, Button } from 'antd';
    ...
    render() {
        ...
        return (
            <div style={ { marginLeft: '5px' } }>
                ...
                <h3>管理联动: { this.dmstore.current_actcode }/{ this.dmstore.current_actname }</h3>

                { this.dmstore.trigger_groups.map((item, idx) =>
                    <Card key={ idx } title={ item.group_name }
                        ...
                        { item.cfgs.map((xitem, idx) =>
                            <Row key={ xitem.id }>
                                ...
                                <Col span={ 3 }>{ xitem.filter_field } { xitem.codetable_category_value }  </Col>
